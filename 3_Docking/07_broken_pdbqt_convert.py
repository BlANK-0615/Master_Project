...
def convert_pdb_to_pdbqt(pdb_file):
    if pdb_file.endswith('.pdb'):
        ...
        # 构造输入文件和输出文件的路径
        input_file = os.path.join('broken_pdb_files', pdb_file)
        output_file = os.path.join('broken_pdbqt_files', f'{basename}.pdbqt')
        try:
            # 运行转换命令
            command = [
            ...
            with open('Broken_PDB_02.txt', 'a') as f:
                f.write(f"Error pdb file at index {pdb_file}\n")

# 创建目标目录
os.makedirs('broken_pdbqt_files', exist_ok=True)

# 列出pdb文件
pdb_files = os.listdir('broken_pdb_files')
