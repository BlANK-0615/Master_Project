...
def dock_decoy(decoy_file, docker_command, receptor_folder, docked_folder, padding):
    ...
    try:
        ...
        print(coordinates)
        with open("bind_site.txt", "a") as bind_site_file:
        ...
        # perform docking
        dock_file(docker_command, receptor_file, decoy_file, *coordinates) # the most costly step :)
        os.makedirs(docked_folder, exist_ok=True)
        origin_path = decoy_file.split('.')[0]
        shutil.move(f"{origin_path}_out.pdbqt", docked_folder)
        with open("progess_rate.txt", "a") as progress_file:
...
decoy_folder = "/home/s2331261/Master_Project/3_Docking/A3_end_broken_file/random_pdbqt_files"
docked_folder = "random_decoy_docked"
padding = 12
