...
def dock_decoy(decoy_file, docker_command, receptor_folder, docked_folder, padding):
    ...
    try:
        ...
        print(coordinates)
        with open("Toco_03_1_bind_site.txt", "a") as bind_site_file:
        ...
        with open("Toco_03_1_progess_rate.txt", "a") as progress_file:
        ...
        with open("Errors_03.txt", "a") as error_file:  # "a" 表示追加模式，新的内容会被写入到文件的末尾
...
docker_command = "/home/s2331261/Msater_Project/dock_job/SCORCH/utils/gwovina-1.0/build/linux/release/gwovina"
receptor_folder = "/home/s2331261/Msater_Project/dock_job/z1_3p_dataset/dock_source/all_receptors_ligands"
decoy_folder = "/home/s2331261/Msater_Project/dock_job/3_Docking/decoy_qdbqt_03_1"
docked_folder = "Toco_docked_03_1"
padding = 12
...
with tqdm(total=len(decoy_files)) as pbar:
    # def update(*a):
    #     pbar.update()
    Parallel(n_jobs=16)(delayed(dock_decoy)(file, docker_command, receptor_folder, docked_folder, padding) for file in decoy_files)
    pbar.update()
#Parallel(n_jobs=32)(delayed(convert_pdb_to_pdbqt)(pdb_file) for pdb_file in tqdm(pdb_files, total=len(pdb_files)))
...
#nohup python3 dock_test_04.py > test1.log&
